...
public class MapTile {
   ...
   // public static int tileW;
    ...
    public void draw(Graphics g){
        //if(tileW <= 0) tileW = tileImg.getWidth(null);
        if(!visible) return;

        if(type == TYPE_BOOM){
            g.drawImage(tileImg[type],x + 10,y + 10,width*10,width*10,null);
        }
        else g.drawImage(tileImg[type],x + 10,y + 10,width,width,null);
    }
